...
# variables
PKG_NAME="${PKG_NAME:-"kubectl-df-pv"}"
PKG_BIN_SRC="${PKG_BIN_SRC:-"df-pv"}"
PKG_REPO="${PKG_REPO:-"yashbhutwala/kubectl-df-pv"}"
...
PKG_DL_URL="https://github.com/${PKG_REPO}/releases/latest/download"
...
if [ "$(bash "${SCRIPT_PATH}/utils.sh" --is-installed ${PKG_BIN})" = "false" ]; then
    ...
    bash "${SCRIPT_PATH}/utils.sh" --sudo-if-needed curl -fLo "${PKG_TMP_ARCHIVE}" "${PKG_DL_URL}/${PKG_SRC_ARCHIVE}" || { echo "ERROR: failed to download package"; exit 1; }
